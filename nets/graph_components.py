...
def conv2d(name, x, maps_in, maps_out, train_phase, size=[3,3], stride=[1,1], act=tf.nn.relu):
    with tf.variable_scope(name):
        w = tf.get_variable(name="conv2d", shape=[size[0], size[1], maps_in, maps_out], initializer=xic())
        c = tf.nn.conv2d(x, w, strides=[1, stride[0], stride[1], 1], padding='SAME')
