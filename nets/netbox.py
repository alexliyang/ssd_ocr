...
from vgg.vgg16 import vgg16 as vgg16
import graph_components as tfc
...
def model(sess):
    ...
    with tf.variable_scope("ssd_extension"):
        ...
        c8_2 = tfc.conv2d("c8_2", c8_1, h[3], h[4], bn, size=[3,3], stride=[2,2])

        c9_1 = tfc.conv2d("c9_1", c8_2, h[4], h[5], bn, size=[1,1])
        c9_2 = tfc.conv2d("c9_2", c9_1, h[5], h[6], bn, size=[3,3], stride=[2,2])

        c10_1 = tfc.conv2d("c10_1", c9_2, h[6], h[7], bn, size=[1,1])
        c10_2 = tfc.conv2d("c10_2", c10_1, h[7], h[8], bn, size=[3,3], stride=[2,2])
    ...
    formatted_outs = tf.concat(outfs, 1)
    print('formatted_outs',formatted_outs)
    #前2个(0-1)
    pred_labels = formatted_outs[:, :, :c_] # lable
    #后4个(2,3,4,5)
    pred_locs = formatted_outs[:, :, c_:] #loc
